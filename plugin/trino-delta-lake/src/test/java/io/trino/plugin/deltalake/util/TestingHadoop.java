...
import io.trino.plugin.deltalake.util.NetworkPortsLock.PortLock;
...
import static io.trino.plugin.deltalake.util.TestContainersUtil.startOrReuse;
        ...
        implements Closeable
{
    ...
    private TestingHadoop(
            ResourceLock<PortLock> portLock,
            String image,
            Set<Integer> ports,
            ...
            int retryLimit)
    {
        if (useFixedPorts) {
            ...
            checkArgument(portLock != null, "Port Lock must be acquired when using fixed ports");
            closer.register(requireNonNull(portLock, "portLock is null"));
        }
        else {
            checkArgument(portLock == null, "Port Lock is not required when not using fixed ports");
        }
        ...
        StartedDockerContainer startedDockerContainer =
                Failsafe.with(retryPolicy).get(() -> startHadoopContainer(image, ports, resourcesToMount, filesToMount, extraHosts, network, useFixedPorts));
        container = startedDockerContainer.getContainer();
        closer.register(startedDockerContainer.getCloser());
    ...
    private StartedDockerContainer startHadoopContainer(
            String image,
            Set<Integer> ports,
            Map<String, String> resourcesToMount,
            Map<String, String> filesToMount,
            ...
            boolean useFixedPorts)
    {
        ...
        Closeable closer = startOrReuse(container);
        return new StartedDockerContainer(container, closer);
    }

    private static class StartedDockerContainer
    {
        private final DockerContainer container;
        private final Closeable closer;

        private StartedDockerContainer(
    ...
    public static final class Builder
    {
        ...
        {
            return new TestingHadoop(null, image, ports, resourcesToMount, filesToMount, extraHosts, network, false, retryLimit);
        }

        public TestingHadoop buildWithFixedPorts(ResourceLock<PortLock> lock)
        {
            return new TestingHadoop(lock, image, ports, resourcesToMount, filesToMount, extraHosts, network, true, retryLimit);
        }
    }
}
